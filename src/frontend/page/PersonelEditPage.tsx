...
import {fetchInsertPatient, fetchUpdatePatient, usePatient} from "../data/patients";
...
export default function PatientEditPage() {
    ...
    async function handleSave(patient: PatientT) {
        try {
            if (is_new) {
                const {patient_id} = await fetchInsertPatient(patient);
                history.push(`/patients/${patient_id}`);

            } else {
                await fetchUpdatePatient(patient_id, patient, cache);
            }
        } catch (e) {
            // TODO
        }
